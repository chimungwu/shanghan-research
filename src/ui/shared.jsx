import { FLAVORS } from "../core/constants.js";
...
export function RadarChart({ fp, fp2, label, label2, size = 280 }) {
  const cx = size / 2;
  const cy = size / 2;
  const R = size * 0.32;
  ...
  const pts2 = fp2 ? getPoints(fp2) : null;
  const pathStr2 = pts2 ? `${pts2.map((p, i) => `${i === 0 ? "M" : "L"}${p[0]},${p[1]}`).join(" ")} Z` : null;
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} style={{ width: "100%", maxWidth: size }}>
      ...
      {pathStr2 && <path d={pathStr2} fill="#3498DB" fillOpacity={0.12} stroke="#3498DB" strokeWidth={1.5} strokeDasharray="4 3" />}
      <path d={pathStr} fill="#C94435" fillOpacity={fp2 ? 0.15 : 0.12} stroke="#C94435" strokeWidth={2} />

      {pts.map((p, i) => (
        <circle key={i} cx={p[0]} cy={p[1]} r={3.5} fill={FLAVOR_COLORS[FLAVORS[i]]} stroke={FLAVORS[i] === "辛" ? "#666" : "#fff"} strokeWidth={1.5} />
      ))}

      {pts2 &&
        pts2.map((p, i) => <circle key={`c${i}`} cx={p[0]} cy={p[1]} r={2.5} fill="#3498DB" stroke="#fff" strokeWidth={1} />)}
      ...
      {FLAVORS.map((f, i) => {
        ...
        return (
          <g key={f}>
            <text x={lx} y={ly - 6} textAnchor="middle" fontSize={14} fontWeight={700} fill={labelColor}>
              {f}
            </text>
            <text x={lx} y={ly + 8} textAnchor="middle" fontSize={10} fill="#999">
              {val > 0 ? `${val}%` : "-"}
            </text>
          </g>
        );
      ...
      {label && (
        <g>
          ...
          <text x={16} y={size - 9} fontSize={9} fill="#888">
            {label}
          </text>
        </g>
      )}
      ...
      {label2 && pts2 && (
        <g>
          ...
          <text x={16} y={size - 21} fontSize={9} fill="#888">
            {label2}
          </text>
        </g>
      )}
...
export function FlavorBar({ fp, height = 24 }) {
    ...
    <div style={{ display: "flex", borderRadius: 4, overflow: "hidden", height, width: "100%", border: "1px solid #e0e0e0" }}>
      {ordered.map((f) => (
        <div
          ...
          style={{
            width: `${fp[f]}%`,
            background: FLAVOR_COLORS[f],
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            fontSize: fp[f] > 12 ? 11 : 0,
            color: f === "辛" || f === "甘" ? "#333" : "#fff",
            fontWeight: 600,
            letterSpacing: 1,
          }}
        >
...
export function NatureGauge({ score }) {
  ...
  return (
    <div
      style={{
        position: "relative",
        height: 12,
        background: "linear-gradient(90deg, #2471A3 0%, #3498DB 25%, #95A5A6 50%, #E67E22 75%, #C0392B 100%)",
        borderRadius: 6,
      }}
    >
      <div
...
export function HerbPill({ herb, expanded, onToggle }) {
  const bg =
    herb.t === "溫"
      ? "#FDEBD0"
      : herb.t === "微溫"
    ...
    <div style={{ display: "inline-block", margin: "2px 3px", verticalAlign: "top" }}>
      <span
        ...
        style={{
          ...
          padding: "3px 10px",
          borderRadius: 20,
          background: expanded ? "#2c3e50" : bg,
          color: expanded ? "#fff" : "inherit",
          fontSize: 13,
          border: expanded ? "1px solid #2c3e50" : "1px solid rgba(0,0,0,0.08)",
          cursor: herb.bt ? "pointer" : "default",
      ...
      >
        ...
        {herb.f.map((f) => (
          <span key={f} style={{ color: expanded ? "#ffd" : FLAVOR_COLORS[f], fontWeight: 700, fontSize: 11 }}>
            {f}
          </span>
        ))}
...
export function FormulaCard({ formula, onClick, selected, isFav, onToggleFav }) {
  return (
    <div
      onClick={onClick}
      style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <span
            style={{
              ...
              borderRadius: 10,
              fontWeight: 600,
              color: TENDENCY_COLORS[formula.nt],
              background: `${TENDENCY_COLORS[formula.nt]}18`,
            }}
          >
            {formula.nt}
          </span>
          <span
            onClick={(e) => {
              e.stopPropagation();
              if (onToggleFav) onToggleFav(formula.n);
            }}
...
// Remaining visualization/detail components are still sourced from index.html in the current migration stage.
